...
export function EditProductPage() {
  ...
  useEffect(() => {
    ...
    getProduct(id).then(p => {
      ...
      if (p.productType === "mix_match") {
        setProductType("mix_match");
        if (p.pricingTiers && p.pricingTiers.length > 0) {
          setPricingTiers(p.pricingTiers.map(t => ({
            id: crypto.randomUUID(),
            quantity: String(t.quantity),
            price: String(t.price),
          })));
        }
        if (p.mixOptions && p.mixOptions.length > 0) {
          setMixOptions(p.mixOptions.map(opt => ({
            id: crypto.randomUUID(),
            value: opt,
            stock: String(p.mixInventory?.[opt] ?? 0),
          })));
        }
        if (p.mixAttributeLabel) setMixAttributeLabel(p.mixAttributeLabel);
        if (p.units) setMixTotalStock(String(p.units));
      } else {
  ...
  const allPreviews = [
    ...existingImageUrls.map((url, idx) => ({ src: url, isExisting: true, idx })),
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {allPreviews.map(({ src, isExisting, idx }) => (
                    <div key={`${isExisting ? "ex" : "new"}-${idx}`} className="relative w-24 h-24 rounded-xl overflow-hidden border bg-muted/30 shrink-0">
                      ...
                      <button type="button" onClick={() => isExisting ? removeExisting(idx) : removeNew(idx)}
                ...
                <p className="text-[11px] text-muted-foreground">First photo is the cover. Tap &times; to remove.</p>
              </div>
              ...
              {productType === "mix_match" && (
                <>
                  ...
                  <div className="border rounded-2xl p-5 space-y-4 bg-muted/20">
                    <div>
                      <p className="text-base font-semibold">Mix Options <span className="text-sm font-normal text-muted-foreground">(optional)</span></p>
                      <p className="text-sm text-muted-foreground mt-0.5">Buyers can pick a combination from these options when ordering.</p>
                    </div>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Tell your customers about this product..."
                      className="min-h-[120px] rounded-xl resize-none" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
